...
function generateDeparture(arrival) {
  ...
  if (!isNaN(flightNo)) {
    if (arrival.companhia.toUpperCase().includes('EMIRATES')) {
      ...
      departure.voo = (flightNo + 1).toString();
      let tatMinutes = 60;
      const ac = arrival.aeronave.toUpperCase();
      if (ac.includes('A320') || ac.includes('B737')) {
        tatMinutes = 45;
      } else if (ac.includes('A330') || ac.includes('B777')) {
        tatMinutes = 60;
      } else if (ac.includes('ATR72') || ac.includes('Q400')) {
        tatMinutes = 25;
      }
